...
# tm_dataset = ProteinDataset('./data/TMfromzb/json/easy.jsonl')
# train_dataloader = DataLoader(tm_dataset, batch_size=2, collate_fn=collate_fn)
# loss=compute_fape
...
tm_dataset = ProteinDataset('./data/TMfromzb/json/content2.jsonl')
...
# model = _load_model("esmfold_3B_v1.pt")
def finetune_esm(model, loss_fn, train_dataset, local_rank=None, device='cuda',  batch_size=BATCH_SIZE, num_epoch=1, lr=0.01):
    model.cuda()
    ...
    for epoch in range(num_epoch):
        ...
        for aatype, mask, residx, target_positions, N_pos_tensor, CA_pos_tensor, C_pos_tensor in tqdm(
            ...
            # loss.requires_grad_(True)   #加入此句就行
            # optimizer.zero_grad()
            # loss.backward()
            # optimizer.step()


finetune_esm(model, loss_fn, train_dataset=tm_dataset)
# finetune_esm(model, loss, train_dataset=tm_dataset, local_rank=args.local_rank)
