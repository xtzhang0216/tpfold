...
from openfold.utils.loss import compute_fape, distogram_loss, ptm_loss, lddt_loss
import noam_opt
...
def run(args, train_set, validation_set, test_set):
    ...
    for epoch in range(args.epoch):
        ...
        for iteration, batch in enumerate(train_loader):
            ...
            name, C_pos, CA_pos, N_pos, CB_pos, seq, mask, residx, bb_pos = batch['name'], batch['C_pos'], batch['CA_pos'], batch['N_pos'], batch['CB_pos'], batch['seq'], batch['mask'], batch['residx'], batch['bb_pos']
            ...
            loss_ptm = ptm_loss(
                logits=output_dict['ptm_logits'],
                pred_affine=output_dict['pred_frames'], 
                backbone_rigid=target_frames, 
                backbone_rigid_mask=output_dict['frame_mask'], 
                resolution=1.5)
            loss_distogram = distogram_loss(
                logits=output_dict['distogram_logits'], 
                pseudo_beta=CB_pos, 
                pseudo_beta_mask=mask)
            # output_dict['lddt_head'] [8,b,l,37,50]
            ...
            backward_loss = (0.5*loss_fape+0.3*loss_distogram+0.01*loss_ptm+0.01*loss_plddt) / args.accumulation_steps
            ...
            loss_distogram_mean = reduce_mean(loss_distogram, dist.get_world_size(), device)
            loss_ptm_mean = reduce_mean(loss_ptm, dist.get_world_size(), device)
            loss_plddt_mean = reduce_mean(loss_plddt, dist.get_world_size(), device)
            
            if args.local_rank == 0:
                metric = {'TRAIN/loss_fape': (loss_fape_mean).item(),
                'TRAIN/loss_distogram': (loss_distogram_mean).item(),
                'TRAIN/loss_ptm': (loss_ptm_mean).item(),
                'TRAIN/loss_plddt': (loss_plddt_mean).item(),
                "TRAIN/loss": (0.5*loss_fape_mean+0.3*loss_distogram_mean+0.01*loss_ptm_mean+0.01*loss_plddt_mean).item(),
                "epoch": epoch}
                wandb.log(metric)
            ...
            total_loss_distogram += loss_distogram_mean.item()
            total_loss_ptm += loss_ptm_mean.item()
            total_loss_plddt += loss_plddt_mean.item()
            ...
            if iteration % log_interval == 0 and iteration > 0 and args.local_rank == 0:
                ...
                cur_loss_distogram = total_loss_distogram / log_interval
                cur_loss_ptm = total_loss_ptm / log_interval
                cur_loss_plddt = total_loss_plddt / log_interval
                ...
                wandb.log({
                    ...
                    "TRAIN/loss_distogram_100steps": cur_loss_distogram,
                    "TRAIN/loss_ptm_100steps": cur_loss_ptm,
                    "TRAIN/loss_plddt_100steps": cur_loss_plddt,
                    "TRAIN/loss_100steps": (0.5*cur_loss_fape+0.3*cur_loss_distogram+0.01*cur_loss_ptm+0.01*cur_loss_plddt),
        ...
        with torch.no_grad():
            ...
            for iteration, batch in enumerate(validation_loader):
                ...
                total_loss_distogram += loss_distogram.item()
                total_loss_ptm += loss_ptm.item()
                total_loss_plddt += loss_plddt.item()

                if args.local_rank == 0:
                    print(f'VAL: | epoch {epoch:3d} | {iteration:5d}/{len(validation_loader):5d} batches | ')
            
            dist.barrier()             
            val_loss_fape = reduce_mean(total_loss_fape, dist.get_world_size(), device) / (iteration+1)
            val_loss_distogram = reduce_mean(total_loss_distogram, dist.get_world_size(), device) / (iteration+1)
            val_loss_ptm = reduce_mean(total_loss_ptm, dist.get_world_size(), device) / (iteration+1)
            val_loss_plddt = reduce_mean(total_loss_plddt, dist.get_world_size(), device) / (iteration+1)
            if args.local_rank == 0 :
                # print(f"val_loss_fape={val_loss_fape}")
                wandb.log({
                    "epoch": epoch,
                    'VAL/fape': val_loss_fape, 
                    'VAL/distogram': val_loss_distogram,
                    'VAL/ptm': val_loss_ptm,
                    'VAL/plddt': val_loss_plddt,
                    'VAL/loss': 0.5*val_loss_fape+0.3*val_loss_distogram+0.01*val_loss_ptm+0.01*val_loss_plddt,
    ...
    with torch.no_grad():
        ...
        dist.barrier()             
        test_loss_fape = reduce_mean(total_loss_fape, dist.get_world_size(), device) / (iteration+1)
        test_loss_distogram = reduce_mean(total_loss_distogram, dist.get_world_size(), device) / (iteration+1)
        test_loss_ptm = reduce_mean(total_loss_ptm, dist.get_world_size(), device) / (iteration+1)
        test_loss_plddt = reduce_mean(total_loss_plddt, dist.get_world_size(), device) / (iteration+1)
    if args.local_rank == 0 :
                # print(f"val_loss_fape={val_loss_fape}")
                wandb.log({
                    'TEST/fape': test_loss_fape, 
                    'TEST/distogram': test_loss_distogram,
                    'TEST/ptm': test_loss_ptm,
                    'TEST/plddt': test_loss_plddt,
                    'TEST/loss': 0.5*test_loss_fape+0.3*test_loss_distogram+0.01*test_loss_ptm+0.01*test_loss_plddt,
            })
