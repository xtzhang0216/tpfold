...
from openfold.utils.loss import compute_fape, distogram_loss
import noam_opt
...
parser.add_argument('--ft_path',type=str,default="/lustre/gst/xuchunfu/zhangxt/checkpoint/230508_token_mlp45/checkpoint-20262/pytorch_model-00002-of-00002.bin",help="jobname of the wandb dashboard")
...
def initial_wandb(args):
    ...
    wandb.init(mode='offline', project=args.project_name, name=args.run_name+str(args.local_rank), config=config)
...
def run(args, train_set, validation_set, test_set):
    ...
    for epoch in range(args.epoch):
        ...
        for iteration, batch in enumerate(train_loader):
            ...
            loss_distogram  = distogram_loss(logits=output_dict['distogram_logits'], pseudo_beta=CB_pos, pseudo_beta_mask=mask)
            ...
            backward_loss = (0.5*loss_fape+0.3*loss_distogram) / args.accumulation_steps
            ...
            if args.local_rank == 0:
                ...
                "TRAIN/loss": (0.5*loss_fape_mean + 0.3*loss_distogram_mean).item(),
                "epoch": epoch}
                wandb.log(metric)
            ...
            if iteration % log_interval == 0 and iteration > 0 and args.local_rank == 0:
                ...
                wandb.log({
                    ...
                    "TRAIN/loss_100steps": 0.5*cur_loss_fape + 0.3*cur_loss_distogram,
        ...
        with torch.no_grad():
            ...
            if args.local_rank == 0 :
                ...
                wandb.log({
                    ...
                    'VAL/loss': 0.5*val_loss_fape + 0.3*val_loss_distogram,
    ...
    if args.local_rank == 0 :
                ...
                wandb.log({
                    ...
                    'TEST/loss': 0.5*test_loss_fape+0.3*test_loss_distogram,
            })
