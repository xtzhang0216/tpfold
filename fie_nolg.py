...
parser.add_argument('--watch_freq',type=int,default=2, help="watch gradient")
...
def main(args):
    ...
    train_set1, validation_set1, test_set1 = [
         ClusteredDataset_inturn(d, max_tokens=args.max_tokens) for d in [dataset, dataset, dataset]]
...
def run(args, train_set, validation_set, test_set):
    ...
    model = nolgfold.load_model(chunk_size=64, pattern=args.pattern)
    ...
    initial_wandb(args)
    wandb.watch(model, log="all", log_freq=args.watch_freq)
