...
class ESMFold(nn.Module):
    ...
    def forward(
        ...
        structure: dict = checkpoint(self.trunk, s_s_0, s_z_0, aa, residx, mask, num_recycles)
...
tm_dataset = ProteinDataset('./data/content2.jsonl')
...
# model = _load_model("/pubhome/bozhang/.cache/torch/hub/checkpoints/esmfold_3B_v1.pt")
# net = model.eval()
model_path = "/pubhome/bozhang/.cache/torch/hub/checkpoints/esmfold_3B_v1.pt"
model_data = torch.load(str(model_path), map_location="cuda:0") #读取一个pickle文件为一个dict
cfg = model_data["cfg"]["model"]
model = TPFold.ESMFold(cfg) # make an instance
model_state = model_data["model"]
model.load_state_dict(model_state, strict=False)
...
# model.to(device)
loss_fn = compute_fape
# with torch.no_grad():
#     model = _load_model("esmfold_3B_v1.pt")
# model = _load_model("esmfold_3B_v1.pt")
def finetune_esm(model, loss, train_dataset, local_rank, device='cuda',  batch_size=BATCH_SIZE, num_epoch=1, lr=0.01):

    model = nn.parallel.DistributedDataParallel(model.cuda(local_rank), device_ids=[local_rank], output_device=local_rank)
    ...
    for epoch in range(num_epoch):
        ...
        # net.train()
        train_acc = 0
        for aatype, mask, residx, target_positions, N_pos_tensor, CA_pos_tensor, C_pos_tensor in tqdm(
            train_dataloader, desc='训练'):
           
            aatype, mask, residx, target_positions, N_pos_tensor, CA_pos_tensor, C_pos_tensor = map(
                lambda x: x.cuda(local_rank),
                (aatype, mask, residx, target_positions, N_pos_tensor, CA_pos_tensor, C_pos_tensor)
            )
            target_frames = Rigid.from_3_points(C_pos_tensor, CA_pos_tensor, N_pos_tensor)
            ...
            # loss_crf = crf(emission, tag, mask=mask)
            # loss = 0.2*loss_crf + loss_fape
            loss = loss_fape
...
finetune_esm(model, loss_fn, train_dataset=tm_dataset, local_rank=args.local_rank)
