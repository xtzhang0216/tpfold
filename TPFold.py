...
class ESMFold(nn.Module):
    def __init__(self, pattern='no', esmfold_config=None, num_tags=6, add_tmbed=False, **kwargs, ):
        super().__init__()
        ...
        if self.cfg["plm_path"] is not None:
            self.esm, self.esm_dict = load_plm(esm2_path=self.cfg["plm_path"]["esm2_path"], ft_path=self.cfg["plm_path"]["ft_path"])
        else:
    ...
    def forward(
        ...
        Args:
            ...
            num_recycles (int): How many recycle iterations to perform. If None, defaults to training max
                recycles, which is 3.
        """  
        # 两种版本输入，全给aa,mask,residx并为tensor，或只给seq,并为str
        ...
        # num_recycles = torch.Tensor([num_recycles])
        # num_recycles.require_grade = True
        # if residx is None:
        #     residx = _residx
        ...
        # print(f"structure aa:{aa.requires_grad}, residx:{residx.requires_grad}\n")
        structure: dict = self.trunk(s_s_0, s_z_0, aa, residx, mask, tmbed_s, self.add_tmbed, num_recycles)

        # structure: dict = checkpoint(self.trunk,
        #  s_s_0, s_z_0, aa, residx, mask)
        ...
        # position,frames = structure["positions"],structure["frames"]
        # print(f"structure position:{position.requires_grad}, frames:{frames.requires_grad}\n")
        ...
        # structure["states"]为8,b,l,384->8,b,l,1850->8,b,l,37,50
        lddt_head = self.lddt_head(structure["states"]).reshape(
            structure["states"].shape[0], B, L, -1, self.lddt_bins
        )
        structure["lddt_head"] = lddt_head 
        # 取lddt_head(8,b,l,37,50)最后一维b,l,37,50, 生成plddt(b,l,37)
        ...
        # print(f"frames: {structure['frames'].require_grad}")
        # print(f"frames: {structure['frames'].require_grad}")
...
def load_model(chunk_size=64, pattern='no', num_tags=6, add_tmbed=False, esmfold_path="/lustre/gst/xuchunfu/zhangxt/.cache/torch/hub/checkpoints/esmfold_3B_v1.pt",esm2_path=None, ft_path=None, cfg=None):
    
    model_data = torch.load(str(esmfold_path), map_location="cpu") #读取一个pickle文件为一个dict
    if cfg == None:
        cfg = model_data["cfg"]["model"]
    if esm2_path != None and ft_path != None:
        cfg["plm_path"] = {"esm2_path":esm2_path, "ft_path":ft_path}
    else:
        cfg["plm_path"] = None
...
def load_model_old(chunk_size=64, pattern='no', num_tags=6, add_tmbed=False, esmfold_path="/lustre/gst/xuchunfu/zhangxt/.cache/torch/hub/checkpoints/esmfold_3B_v1.pt",esm2_path=None, ft_path=None, cfg=None):
