...
def _load_model(model_name, chunk_size=64, pattern="no"):
    ...
    model = ESMFold(esmfold_config=cfg, pattern=pattern)
    model.load_state_dict(model_state, strict=False)
    model.set_chunk_size(chunk_size)
    return model

# model = _load_model("esmfold_3B_v1.pt")
# def load_model(chunk_size, pattern='no', num_tags=5, model_path="/pubhome/bozhang/.cache/torch/hub/checkpoints/esmfold_3B_v1.pt"):

    
    # model_data = torch.load(str(model_path), map_location="cuda:0") #读取一个pickle文件为一个dict
    # cfg = model_data["cfg"]["model"]
    # model = ESMFold(pattern=pattern, num_tags=num_tags,esmfold_config=cfg) # make an instance
    # model_state = model_data["model"]
    # model.load_state_dict(model_state, strict=False)
    # model.set_chunk_size(chunk_size)
...
def load_model(chunk_size, pattern='no', num_tags=6, add_tmbed=False, model_path="/pubhome/bozhang/.cache/torch/hub/checkpoints/esmfold_3B_v1.pt", cfg=None):
